...
//                .doOnNext(r-> r.headers().forEach(h->log.info("header key: {} , value: {}", h.key(), new String(h.value())))) //printhing the header typeid
                .doOnNext(r-> log.info("key: {}, value: {}", r.key(), r.value().customerId())) //get custom object value -customerId
                .subscribe();
    }
}
